...
@Service
@Slf4j
public class YkLczh1Service {
    private Integer gameCode = 10;
    ...
    public void gameInfo() throws Exception {
        ...
        JSONObject result = ykService.requestData(null, gameCode,"灵宠召唤-游戏信息");
        if (Objects.nonNull(result)) {
            ...
            redisService.setCacheObject("gameInfo:lczh", params.toJSONString());
            TimerFactory.getTimer().schedule(new Lczh1DataTask(2, this, 0), nextDate);
        } else {
            TimerFactory.getTimer().schedule(new Lczh1DataTask(1, this), 1000L);
        }
    }
    ...
    public void luckMonster(int times) throws Exception {
        if (times > 25) {
            ...
            JSONObject result = ykService.requestData(this.openIndex, this.gameCode,"灵宠召唤-开奖结果");
            if (Objects.nonNull(result)) {
                    ...
                    if (StringUtils.isNotBlank(number) && !Objects.equals("x", number.toLowerCase())) {
                        ...
                        for (int i = 0; i < DomainNameUtil.urls.length; i++) {
                            ...
                            try {
                                ...
                                log.info(url + "-映客-灵宠召唤-开奖结果同步请求响应：{}", resp);
                            } catch (RestClientException e) {
                                log.info(url + "-映客-灵宠召唤-开奖结果同步请求异常：{}", e.getMessage());
                            }
                        }
                        TimerFactory.getTimer().schedule(new Lczh1DataTask(1, this), 200);
                        successStatus = true;
                    }
                }
