...
public class LczhService {
    ...
    public void gameInfo() throws InterruptedException {
        ...
        this.getTimeInfoTimes = System.currentTimeMillis();// 记录运行的时间戳，方便检测死机重启
        Map<String, Object> data = new HashMap<>();
        data.put("lotteryId", 22);
        HttpResponse execute = HttpUtil.createPost(ymzxDomain + "/game/lottery/GetIndex")
                .header("token", ympdService.getToken())
                .form(data).execute();
        log.info("元梦之星-灵宠召唤-游戏信息请求 返回参数 {}", execute.body());
        if (execute.isOk() && StringUtils.isNotBlank(execute.body())) {
            ...
            if (success == 1) {
                ...
                TimerFactory.getTimer().schedule(new LczhDataTask(2, this, 0), nextDate);
                redisService.setCacheObject("gameInfo:lczh", info.toJSONString());
            } else {
                log.info("元梦之星-灵宠召唤-游戏信息请求失败 请求参数 {}", JSONObject.toJSONString(data));
                TimerFactory.getTimer().schedule(new LczhDataTask(1, this), 1000L);
            }
        } else {
            TimerFactory.getTimer().schedule(new LczhDataTask(1, this), 1000L);
        }
    ...
    public void luckMonster(int times) throws InterruptedException {
        if (times > 18) {
            ...
            HttpResponse execute = HttpUtil.createPost(ymzxDomain + "/game/lottery/GetOpenNumber")
                    .header("token", ympdService.getToken())
                    .form(data).execute();
            log.info("元梦之星-灵宠召唤-开奖结果请求 {} 返回参数 {}", times, execute.body());
            if (execute.isOk() && StringUtils.isNotBlank(execute.body())) {
                JSONObject result = JSONObject.parseObject(execute.body());
                int success = result.getIntValue("success");
                if (success == 1) {
                    ...
                    if (StringUtils.isNotBlank(number) && !Objects.equals("x", number.toLowerCase())) {
                        ...
                        for (int i = 0; i < DomainNameUtil.urls.length; i++) {
                            String url= DomainNameUtil.urls[i]+"/lczh/luckyMonster";
                            try {
                                ResponseEntity<String> responseEntity = RestTemplateUtils.post(url, info.toJSONString(), String.class);
                                String resp = responseEntity.getBody();
                                log.info(url + "-元梦之星-灵宠召唤-开奖结果同步请求响应：{}", resp);
                            } catch (RestClientException e) {
                                log.info(url+ "-元梦之星-灵宠召唤-开奖结果同步请求异常：{}", e.getMessage());
                            }
                        }
                        TimerFactory.getTimer().schedule(new LczhDataTask(1, this), 200);
                        successStatus = true;
                    }

                } else {
                    log.info("元梦之星-灵宠召唤-开奖结果请求失败 请求参数 {}", JSONObject.toJSONString(data));
                }
            } else {
                log.info("元梦之星-灵宠召唤-开奖结果请求失败 请求参数 {}", JSONObject.toJSONString(data));
            }
